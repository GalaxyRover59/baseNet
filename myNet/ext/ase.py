...
class Relaxer:
    ...
    def relax(
        ...
        """
        is_Molecule = True if isinstance(atoms, Molecule) else False
        ...
        with contextlib.redirect_stdout(stream):
            if is_Molecule:
                obs = MoleculeTrajectoryObserver(atoms)
            else:
        ...
        return {
            "final_structure": self.ase_adaptor.get_molecule(atoms) if is_Molecule else self.ase_adaptor.get_structure(
                atoms),
            "trajectory": obs,
        }
...
class MoleculeTrajectoryObserver(collections.abc.Sequence):
    """Trajectory observer is a hook in the relaxation process that saves the
    intermediate structures for molecules.
    """
    ...
    def __getitem__(self, item):
        return self.energies[item], self.forces[item], self.atom_positions[item]
